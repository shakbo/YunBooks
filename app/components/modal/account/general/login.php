    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="loginForm" method="post" action="app/scripts/account/general/login.php" class="needs-validation" novalidate>
                    <div class="mb-3 has-validation">
                        <label for="loginInputEmail" class="form-label">電子郵件地址</label>
                        <input type="email" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$" class="form-control" id="loginInputEmail" name="email" data-v-min-length="3" data-v-message="電子郵件不符合格式" required>
                    </div>
                    <div class="mb-3">
                        <label for="loginInputPassword" class="form-label has-validation">密碼</label>
                        <input type="password" pattern="(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])\S{6,}" class="form-control" id="loginInputPassword" name="password" data-v-message="長度需大於六個字，並包含大小寫英文及數字。" required>
                    </div>
